...
###########################################################################
# Example Hydroelastic Cube Lifting
#
# Shows how to use Newton to simulate a cube lifting from a table.
#
# Commands: python -m newton.examples hydroelastic_cube_lifting
#           uv run -m newton.examples hydroelastic_cube_lifting
#
# Optional arguments:
# --viewer=usd --output-path hydroelastic_cube_lifting.usd
#
###########################################################################
...
class Example:
    def __init__(self, viewer, verbose=False):
        ...
        self.sim_time = 0.0
        self.sim_substeps = 200  # Use 100 substeps with fps = 100 to avoid slippage.
        ...
        # max_geom_pairs = [768 * 12] * len(self.model.collision_pairs)
        # max_geom_pairs[2] = 768 * 200
    ...
    def load_model(self):
        ...
        object_com_height = 0.5 * 0.1
        # ========================================================================
        # TODO: Refactor variable names for clarity.
        finger_com_height = 0.5 * 0.018
        contact_length = object_com_height + finger_com_height

        object_up = table_offset + (table_com_height + object_com_height)
        object_front = -3.0 * object_com_height
        ...
        body_idx = 0
        table = body_idx
        poses[body_idx, :3] = 0.0 * dirs.up
        poses[body_idx, 3:] = unit_q
        # Cube 0
        body_idx += 1
        cube_0 = body_idx
        poses[body_idx, :3] = object_up * dirs.up
        poses[body_idx, 3:] = unit_q
        # Cube 1
        body_idx += 1
        cube_1 = body_idx
        poses[body_idx, :3] = object_up * dirs.up + object_front * dirs.front
        poses[body_idx, 3:] = unit_q
        # Gripper base
        body_idx += 1
        gripper_base = body_idx
        poses[body_idx, :3] = (finger_y + tool_offset) * dirs.up + object_front * dirs.front
        poses[body_idx, 3:] = unit_q
        # Finger 0 (This one gets overriden when adding the joint for finger 0)
        body_idx += 1
        finger_0 = body_idx
        poses[body_idx, :3] = poses[gripper_base, :3] + (-tool_offset * dirs.up - 0.075 * dirs.right)
        poses[body_idx, 3:] = unit_q
        # Finger 1 (This one gets overriden when adding the joint for finger 1)
        body_idx += 1
        finger_1 = body_idx
        poses[body_idx, :3] = poses[gripper_base, :3] + (-tool_offset * dirs.up + 0.075 * dirs.right)
        poses[body_idx, 3:] = unit_q
        ...
        # Add joints
        xform = wp.transform(wp.vec3f(table_offset * dirs.up), wp.quat_identity())
        scene.add_joint_fixed(parent=-1, child=table, parent_xform=xform)
        scene.add_joint_free(parent=-1, child=cube_0)
        scene.add_joint_free(parent=-1, child=cube_1)

        scene.add_joint_d6(
            parent=-1,
            child=gripper_base,
            parent_xform=wp.transform(wp.vec3(poses[gripper_base, :3]), wp.quat_identity()),
            linear_axes=[
    ...
    def simulate(self):
        ...
        for i in range(self.sim_substeps):
            ...
            # hydroelastic_isosurface.compute_contact_surfaces_and_wrenches(
            #     self.solver,
            #     self.state_0,
            #     self.contacts,
            #     self.twist_convention,
    ...
    def compute_control(self):
        ...
        # ========================================================================
        t_close_fingers = 2.0
        t_lift = t_close_fingers + 0.5
        t_align = t_lift + 1.0
        t_pause_after_align = t_align + 0.5
        t_down = t_pause_after_align + 0.5
        t_release = t_down + 0.1

        freq = np.pi
        target_x = 0.06
        lift_height = 0.15
        align_distance = 3 * 0.05
        down_distance = 0.05
        joint_offset = 2 * 6
        if sim_time < t_close_fingers:
            # Close fingers
            # Compute joint targets.
            joint_offset += 3
            joint_target[joint_offset + 0] = target_x * sim_time / t_close_fingers
            joint_target[joint_offset + 1] = -joint_target[joint_offset + 0]
        elif sim_time < t_lift:
            # Lift cube
            # This should have max vel of  0.02*0.5*pi/(1.0-0.5)) or approx 61mm/s
            # Compute normalized time t that goes from 0 to 1.
            t = (sim_time - t_close_fingers) / (t_lift - t_close_fingers)
            # Compute interpolation factor alpha that goes from 0 to 1.
            alpha = 0.5 * (1.0 - np.cos(freq * t))
            # Compute joint targets.
            joint_target[joint_offset + 2] = lift_height * alpha

            # Ensure that the fingers are closed.
            joint_offset += 3
            joint_target[joint_offset + 0] = target_x
            joint_target[joint_offset + 1] = -joint_target[joint_offset + 0]
        elif sim_time < t_align:
            # Ensure that finger are lifted.
            joint_target[joint_offset + 2] = lift_height

            # Align
            # Compute normalized time t that goes from 0 to 1.
            t = (sim_time - t_lift) / (t_align - t_lift)
            # Compute interpolation factor alpha that goes from 0 to 1.
            alpha = 0.5 * (1.0 - np.cos(freq * t))
            # Compute joint targets.
            joint_target[joint_offset + 0] = align_distance * alpha
        elif sim_time < t_pause_after_align:
            # Ensure that fingers are aligned.
            joint_target[joint_offset + 0] = align_distance
        elif sim_time < t_down:
            # Down
            # Compute normalized time t that goes from 0 to 1.
            t = (sim_time - t_pause_after_align) / (t_down - t_pause_after_align)
            # Compute interpolation factor alpha that goes from 0 to 1.
            alpha = 0.5 * (1.0 - np.cos(freq * t))
            # Compute joint targets.
            joint_target[joint_offset + 2] = lift_height - down_distance * alpha

        elif sim_time < t_release:
            # Ensure that fingers are down.
            joint_target[joint_offset + 2] = lift_height - down_distance

            # Release
            joint_offset += 3
            t = (sim_time - t_down) / (t_release - t_down)
            joint_target[joint_offset + 0] = target_x - target_x * t
            joint_target[joint_offset + 1] = -joint_target[joint_offset + 0]
        else:
            joint_offset += 3
            joint_target[joint_offset + 0] = 0
            joint_target[joint_offset + 1] = 0

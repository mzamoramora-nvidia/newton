...
###########################################################################
# Example Hydroelastic Ball vs Floor
#
# Shows how to use Newton to simulate a ball bouncing on a floor.
#
...
#
# Commands: python -m newton.examples hydroelastic_basic_ball_vs_floor
#           uv run -m newton.examples hydroelastic_basic_ball_vs_floor
#
# Optional arguments:
# --viewer=usd --output-path hydroelastic_basic_ball_vs_floor.usd
###########################################################################
...
class Example:
    def __init__(self, viewer, verbose=False):
        ...
        self.sim_time = 0.0
        self.sim_substeps = 10  # Use 100 substeps with fps = 100 to avoid slippage.
    ...
    def load_model(self):
        ...
        body_idx = 0
        table = body_idx
        poses[body_idx, :3] = 0.2 * self.dirs.up
        poses[body_idx, 3:] = q

        # Ball
        body_idx += 1
        ball = body_idx
        poses[body_idx, :3] = 0.42 * self.dirs.up
        poses[body_idx, 3:] = q
        ...
        scene.add_joint_free(parent=-1, child=ball)
        ...
        # ==============================================================================================================
        # Set collision pairs.
        model.collision_pairs = [[ball, table]]
    ...
    def simulate(self):
        ...
        for _i in range(self.sim_substeps):
            ...
            #     update_contact_pairs=(i % 2 == 0 or i == self.sim_substeps - 1),
            # )
            hydroelastic_isosurface.batch_compute_contact_surfaces_and_wrenches(
                self.solver,
                self.state_0,

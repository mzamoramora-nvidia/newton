...
class Example:
    ...
    def load_model(self):
        ...
        poses = np.zeros((5, 7), dtype=np.float32)
        # Table
        body_idx = 0
        table = body_idx
        ...
        tool_offset = 0.10
        poses[body_idx, :3] = (finger_y + tool_offset) * dirs.up
        poses[body_idx, 3:] = q
        # Finger 0 (This one gets overriden when adding the joint for finger 0)
        body_idx += 1
        finger_0 = body_idx
        poses[body_idx, :3] = finger_y * dirs.up - 0.1 * dirs.front
        ...
        scene.add_joint_d6(
            parent=-1,
            child=gripper_base,
            parent_xform=wp.transform(wp.vec3((finger_y + tool_offset) * dirs.up), wp.quat_identity()),
            linear_axes=[
        ...
        pos = -0.10 * dirs.right - tool_offset * dirs.up
        scene.add_joint_d6(
            parent=gripper_base,
            child=finger_0,
        ...
        # ==============================================================================================================
        # Set collision pairs.
        model.collision_pairs = [[table, cube], [finger_0, cube], [finger_1, cube]]
    ...
    def compute_control(self):
        ...
        joint_offset = 6
        if sim_time < t1:
            # This should have max vel of  0.06*0.5*pi/(2.0)) or approx 47mm/s
            freq = np.pi
            alpha = 0.5 * (1.0 - np.cos(freq * sim_time / t1))
            joint_target[joint_offset + 1] = target_x * sim_time / t1  # *alpha
            joint_target[joint_offset + 2] = -joint_target[joint_offset + 1]
        elif sim_time < t2:
            # This should have max vel of  0.02*0.5*pi/(1.0-0.5)) or approx 61mm/s
            # This should have max vel of  0.2*0.5*pi/(2.5-2.0)) or approx 628mm/s
            freq = np.pi
            alpha = 0.5 * (1.0 - np.cos(freq * (sim_time - t1) / (t2 - t1)))
            joint_target[joint_offset + 0] = lift_height * alpha
            joint_target[joint_offset + 1] = target_x
            joint_target[joint_offset + 2] = -joint_target[joint_offset + 1]
        else:
            joint_target[joint_offset + 0] = lift_height

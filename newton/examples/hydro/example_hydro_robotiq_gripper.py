...
def control_pipeline_kernel(
    ...
    manual_mode: wp.array[int],
    # GUI inputs (manual mode, GPU buffers)
    gui_pos: wp.array[wp.vec3],
    gui_rot: wp.array[wp.vec3],
    gui_ctrl: wp.array[float],
    # State machine state (auto mode, read/write)
    task: wp.array[int],
    task_timer: wp.array[float],
    # State machine config
    grasp_z: float,
    lift_z: float,
    grasp_ctrl: float,
    task_durations: wp.array[float],
    frame_dt: float,
    hold_rot: wp.vec3,
    # Rate limiter state (read/write)
    prev_base_target: wp.array2d[float],
    prev_gripper_ctrl: wp.array[float],
    # Rate limit config (GPU buffers, written from CPU on slider change)
    vel_limits: wp.array[wp.vec3],  # (1,): [pos_max_delta, rot_max_delta, gripper_max_delta]
    # Current joint state (for position error check)
    joint_q: wp.array[float],
    joint_coord_count: int,
    # Outputs
    joint_target_pos: wp.array[float],
    direct_control: wp.array2d[float],
    dofs_per_world: int,
...
_AXIS_VECTORS = [wp.vec3(1, 0, 0), wp.vec3(0, 1, 0), wp.vec3(0, 0, 1)]


def _pad_normal_from_body_q(body_q_7, pad_thin_axis: int) -> np.ndarray:
    """Get pad face normal in world frame from a body_q [x,y,z, qx,qy,qz,qw] vector."""
    q = wp.quat(float(body_q_7[3]), float(body_q_7[4]), float(body_q_7[5]), float(body_q_7[6]))
    n = wp.quat_rotate(q, _AXIS_VECTORS[pad_thin_axis])
    return np.array([float(n[0]), float(n[1]), float(n[2])])


def _compute_pad_metrics(left_q, right_q, pad_thin_axis: int) -> dict:
    """Compute pad parallelism metrics from left/right pad body_q vectors.

    Returns dict with pad_angle_deg, left_pad_tilt_deg, right_pad_tilt_deg.
    """
    ln = _pad_normal_from_body_q(left_q, pad_thin_axis)
    rn = _pad_normal_from_body_q(right_q, pad_thin_axis)

    # Pad-to-pad angle
    cos_angle = np.clip(np.dot(ln, rn), -1.0, 1.0)
    pad_angle = math.degrees(math.acos(abs(cos_angle)))

    # Tilt from vertical (pad normal should be horizontal -> dot with Z should be ~0)
    l_tilt = math.degrees(math.asin(np.clip(abs(ln[2]), 0.0, 1.0)))
    r_tilt = math.degrees(math.asin(np.clip(abs(rn[2]), 0.0, 1.0)))

    return {
        "pad_angle_deg": pad_angle,
        "left_pad_tilt_deg": l_tilt,
        "right_pad_tilt_deg": r_tilt,
    }
...
class DiagnosticsLogger:
    ...
    def __init__(
        ...
        left_pad_body_idx: int | None = None,
        right_pad_body_idx: int | None = None,
        pad_thin_axis: int = 0,
    ):
        ...
        self.left_pad_body_idx = left_pad_body_idx
        self.right_pad_body_idx = right_pad_body_idx
        self.pad_thin_axis = pad_thin_axis
    ...
    def sample_substeps(
        ...
        for s in range(num_substeps):
            ...
            for w in range(self.num_worlds):
                ...
                # Pad parallelism metrics
                if self.left_pad_body_idx is not None and self.right_pad_body_idx is not None:
                    left_q = bq[w, self.left_pad_body_idx, :]
                    right_q = bq[w, self.right_pad_body_idx, :]
                    pad_metrics = _compute_pad_metrics(left_q, right_q, self.pad_thin_axis)
                    rec.update(pad_metrics)
...
class Example:
    def __init__(self, viewer, num_worlds=1, args=None):
        ...
        # ---- Debug: toggle individual modifications (for bisecting pad tilt issue) ----
        self._disable_tendon_override = getattr(args, "disable_tendon_override", False) if args else False
        ...
        self.sdf_params = {
            "max_resolution": 64,
            "narrow_band_range": (-0.0015, 0.0015),
        }

        # ---- Build model ----
        builder = newton.ModelBuilder()
        builder.rigid_gap = self.shape_cfg.gap  # 5mm default gap (instead of 100mm)
        ...
        # Cache pad body indices (within a single world) for parallelism diagnostics
        self.left_pad_body_idx = None
        self.right_pad_body_idx = None
        for i, label in enumerate(builder.body_label):
            if label.endswith("/left_pad") or label == "left_pad":
                self.left_pad_body_idx = i
            elif label.endswith("/right_pad") or label == "right_pad":
                self.right_pad_body_idx = i
        ...
        self.grasp_z = self.table_height + self.gripper_tcp_closed + 0.002
        ...
        # ---- Diagnostics logger (requires solver and pad body indices) ----
        # Determine pad thin axis from MuJoCo model geom sizes
        pad_thin_axis = 0  # default
        if self.diagnostics:
            if hasattr(self.solver, "mj_model"):
                import mujoco as _mj  # noqa: PLC0415

                mj_model = self.solver.mj_model
                for g in range(mj_model.ngeom):
                    name = _mj.mj_id2name(mj_model, _mj.mjtObj.mjOBJ_GEOM, g)
                    if name and "pad1" in name:
                        pad_thin_axis = int(np.argmin(mj_model.geom_size[g]))
                        break
            self.diag_logger = DiagnosticsLogger(
                self.num_worlds,
                left_pad_body_idx=self.left_pad_body_idx,
                right_pad_body_idx=self.right_pad_body_idx,
                pad_thin_axis=pad_thin_axis,
            )
        else:
    ...
    def _add_gripper(self, builder):
        ...
        if not self._disable_tendon_override:
            builder.custom_attributes["mujoco:tendon_coef"].values = [0.485, 0.485]
        if not self._disable_forcerange_override:
        ...
        if not self._disable_armature_scale:
            gripper_dof_offset = self.gripper_joints_start
            gripper_dof_count = builder.joint_dof_count - gripper_dof_offset
            if gripper_dof_count == 8:
                mjcf_armature = np.array([0.005, 0.001, 0.001, 0.001, 0.005, 0.001, 0.001, 0.001])
            else:
                # Without coupler joints: 6 DOFs (driver, spring_link, follower per side)
                mjcf_armature = np.array([0.005, 0.001, 0.001, 0.005, 0.001, 0.001])
            armature_scale = 2.0
            gripper_armature = (armature_scale * mjcf_armature).tolist()
            builder.joint_armature[gripper_dof_offset : gripper_dof_offset + len(gripper_armature)] = gripper_armature
            print(f"Gripper armature (scale={armature_scale:.1f}x): {[f'{a:.4f}' for a in gripper_armature]}")

    def _add_scene_shapes(self, builder: newton.ModelBuilder):
        ...
        object_inertia = wp.mat33(np.eye(3, dtype=np.float32)) * self.object_armature
        self.object_body_idx = builder.add_body(xform=object_xform, label="grasp_object", inertia=object_inertia)
    ...
    def step(self):
        ...
        # Debug: log control outputs for first few frames
        if hasattr(self, "_debug_ctrl") and self._frame_count < 20:
            tp = self.control.joint_target_pos.numpy()
            mc = self.control.mujoco.ctrl.numpy() if self.has_mujoco_ctrl else []
            task = self.task.numpy()
            prev = self.prev_base_target.numpy()
            prev_gc = self.prev_gripper_ctrl.numpy()
            print(
                f"  [ctrl] frame={self._frame_count} task={task[0]} "
                f"target_z={tp[2]:.4f} ctrl={mc[0] if len(mc) > 0 else 0:.2f} "
                f"prev_z={prev[0, 2]:.4f} prev_ctrl={prev_gc[0]:.2f}"
            )
    ...
    def test_final(self):
        ...
        # Pad parallelism report
        if self.left_pad_body_idx is not None and self.right_pad_body_idx is not None:
            body_q = self.state_0.body_q.numpy().reshape(self.num_worlds, self.bodies_per_world, 7)
            pad_thin_axis = self.diag_logger.pad_thin_axis if self.diag_logger else 0
            print("\n  Pad parallelism (current frame):")
            for w in range(self.num_worlds):
                left_q = body_q[w, self.left_pad_body_idx, :]
                right_q = body_q[w, self.right_pad_body_idx, :]
                metrics = _compute_pad_metrics(left_q, right_q, pad_thin_axis)
                angle = metrics["pad_angle_deg"]
                l_tilt = metrics["left_pad_tilt_deg"]
                r_tilt = metrics["right_pad_tilt_deg"]
                warn = " *** WARNING" if angle > 2.0 else ""
                print(f"    World {w}: pad_angle={angle:.2f}° L_tilt={l_tilt:.2f}° R_tilt={r_tilt:.2f}°{warn}")

            # If diagnostics were running, report max across all substeps
            if self.diag_logger and self.diag_logger.substep_records:
                print("  Pad parallelism (max across all substeps):")
                for w in range(self.num_worlds):
                    world_recs = [
                        r for r in self.diag_logger.substep_records if r.get("world") == w and "pad_angle_deg" in r
                    ]
                    if world_recs:
                        max_angle = max(r["pad_angle_deg"] for r in world_recs)
                        max_l = max(r["left_pad_tilt_deg"] for r in world_recs)
                        max_r = max(r["right_pad_tilt_deg"] for r in world_recs)
                        warn = " *** WARNING" if max_angle > 2.0 else ""
                        print(
                            f"    World {w}: max_pad_angle={max_angle:.2f}° max_L={max_l:.2f}° max_R={max_r:.2f}°{warn}"
                        )
...
if __name__ == "__main__":
    ...
    # Debug toggles for bisecting pad tilt issue
    parser.add_argument("--disable-tendon-override", action="store_true", help="Skip tendon coef override.")
    parser.add_argument(
        "--disable-forcerange-override", action="store_true", help="Skip actuator force range override."
    )
    ...
    parser.add_argument("--debug-ctrl", action="store_true", help="Log control values for first 20 frames.")

...
def hydroelastic_stiffness_factor_kernel(
    shape_material_kh: wp.array[wp.float32],
    contact_count: wp.array[int],
    contact_shape0: wp.array[int],
    contact_shape1: wp.array[int],
    contact_stiffness: wp.array[wp.float32],
    out_stiffness_factor: wp.array[wp.float32],
):
    """Cache the non-differentiable geometry factor for hydroelastic stiffness."""
    tid = wp.tid()
    count = contact_count[0]
    if tid >= count:
        return
    ...
    shape_a = contact_shape0[tid]
    shape_b = contact_shape1[tid]
    factor = wp.float32(0.0)
    if shape_a >= 0 and shape_b >= 0:
        k_eff = _effective_stiffness(shape_material_kh[shape_a], shape_material_kh[shape_b])
        if k_eff > 0.0:
            factor = contact_stiffness[tid] / k_eff

    out_stiffness_factor[tid] = factor


@wp.kernel
def differentiable_hydroelastic_stiffness_kernel(
    shape_material_kh: wp.array[wp.float32],
    contact_count: wp.array[int],
    contact_shape0: wp.array[int],
    contact_shape1: wp.array[int],
    contact_stiffness_factor: wp.array[wp.float32],
    out_stiffness: wp.array[wp.float32],
):
    """Replay hydroelastic per-contact stiffness through differentiable ``kh``."""
    tid = wp.tid()
    count = contact_count[0]
    if tid >= count:
        return
    ...
    if shape_a >= 0 and shape_b >= 0:
        k_eff = _effective_stiffness(shape_material_kh[shape_a], shape_material_kh[shape_b])
        stiffness = contact_stiffness_factor[tid] * k_eff

    out_stiffness[tid] = stiffness


def launch_differentiable_hydroelastic_stiffness(
    contacts,
    shape_material_kh: wp.array,
    device=None,
):
    """Populate hydroelastic contact stiffness through differentiable ``kh``.

    Hydroelastic contact geometry is generated by tape-disabled kernels. This
    first freezes the geometry factor from the current contacts, then replays
    stiffness in a tape-recorded kernel so material identification can optimize
    ``shape_material_kh``.
    """
    if contacts.rigid_contact_stiffness is None or contacts.rigid_contact_stiffness_factor is None:
        return

    wp.launch(
        kernel=hydroelastic_stiffness_factor_kernel,
        dim=contacts.rigid_contact_max,
        inputs=[
            shape_material_kh,
            contacts.rigid_contact_count,
        ...
        outputs=[contacts.rigid_contact_stiffness_factor],
        device=device,
        record_tape=False,
    )

    wp.launch(
        kernel=differentiable_hydroelastic_stiffness_kernel,
        dim=contacts.rigid_contact_max,
        inputs=[
            shape_material_kh,
            contacts.rigid_contact_count,
            contacts.rigid_contact_shape0,
            contacts.rigid_contact_shape1,
            contacts.rigid_contact_stiffness_factor,
        ],
        outputs=[contacts.rigid_contact_stiffness],
        device=device,
    )

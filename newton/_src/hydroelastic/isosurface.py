...
# These values match those used in Drake's mesh_intersection.cc implementation
...
@wp.kernel
def compute_hydroelastic_contact_surface_elements(
    ...
    surfaces between pairs of tetrahedra from two different meshes. Each
    thread processes one tetrahedron pair.
    """
    tid = wp.tid()

    tet_idx_a = tet_pairs[tid][0]
    tet_idx_b = tet_pairs[tid][1]
...
def launch_compute_soft_vs_hard_contact_surface(body_q, body_q_inv_mat, mesh_a, mesh_b, isosurface):
    wp.launch(
...
def compute_contact_polygons(
    ...
    #     body_q=body_q,
    #     body_q_inv_mat=body_q_inv_mat,
    #     mesh_a=mesh_a,
    #     mesh_b=mesh_b,
    #     isosurface=isosurface,
    # )

    # num_polygon_vertices = wp.zeros(1, dtype=wp.int32, device=compute_device)
    # wp.launch(
    #     sum_array_int32,
    #     dim=1,
    #     inputs=[
    #         contact_polygon_vertex_counts,
    #     ],
    #     outputs=[
    ...
    # # Process results and build triangulated surface
    # polygon_vertex_counts_np = contact_polygon_vertex_counts.numpy()
    # total_polygon_vertices = int(np.sum(polygon_vertex_counts_np))

    # if total_polygon_vertices == 0:
    #     return (
    #         False,
    #         None,
    #         None,
    ...
    #         None,
    #         None,
    #         intermediate_contact_polygon_vertices,
    #         intermediate_contact_polygon_vertex_counts,
    #         intermediate_contact_polygon_centroids,
    #         contact_polygon_vertices,
    #         contact_polygon_vertex_counts,
    #         contact_polygon_centroids,
    #         contact_pressure_values,
    #     )

    # # Find valid polygons and compute indexing
    # vertex_offset_cumulative_np = np.cumsum(polygon_vertex_counts_np)
    # valid_polygon_indices_np = np.nonzero(polygon_vertex_counts_np)[0]
    # num_valid_polygons = len(valid_polygon_indices_np)

    # # polygon_areas_np = polygon_areas.numpy()[valid_polygon_indices_np]
    # # print("Polygon areas:")
    # # print(polygon_areas_np)
    # # print(f"Min max area: {np.min(polygon_areas_np)}, {np.max(polygon_areas_np)}")

    # # Create triangulated surface representation
    # vertex_offset_cumulative = wp.array(vertex_offset_cumulative_np, dtype=wp.int32, device=compute_device)
    # valid_polygon_indices = wp.array(valid_polygon_indices_np, dtype=wp.int32, device=compute_device)

    # # Allocate output arrays
    # total_vertices = total_polygon_vertices + num_valid_polygons  # polygon vertices + centroids
    # triangulated_vertices = wp.zeros(total_vertices, dtype=wp.vec3f, device=compute_device)
    # triangulated_elements = wp.zeros(total_polygon_vertices, dtype=wp.vec3i, device=compute_device)
    # per_triangle_pressure = wp.zeros(total_polygon_vertices, dtype=wp.float32, device=compute_device)
    # visualization_radii = wp.zeros(total_polygon_vertices, dtype=wp.vec2i, device=compute_device)
    # visualization_edges = wp.zeros(total_polygon_vertices, dtype=vec4i, device=compute_device)
    # per_triangle_tet_pairs = wp.zeros(total_polygon_vertices, dtype=wp.vec2i, device=compute_device)

    # # Generate triangulated surface
    # wp.launch(
    #     generate_contact_surface_triangulation,
    #     dim=num_valid_polygons,
    #     inputs=[
    ...
    #         contact_polygon_centroids,
    #         contact_polygon_vertex_counts,
    #         contact_surface_normals,
    #         contact_pressure_values,
    #         vertex_offset_cumulative,
    #         total_polygon_vertices,
    #     ],
    #     outputs=[
    #         triangulated_vertices,
    #         triangulated_elements,
    #         per_triangle_pressure,
    #         visualization_radii,
    #         visualization_edges,
    #         per_triangle_tet_pairs,
    #     ],
    # )

    # # Extract results for valid polygons
    # valid_surface_normals = contact_surface_normals.numpy()[valid_polygon_indices_np]
    # valid_surface_centroids = contact_polygon_centroids.numpy()[valid_polygon_indices_np]
    ...
    ## Old return would map to:
    # (
    #     self.isosurfaces[i].is_valid,
    #     self.isosurfaces[i].vertices,
    #     self.isosurfaces[i].triangles,
    #     self.isosurfaces[i].per_triangle_pressure,
    #     self.isosurfaces[i].per_triangle_tet_pairs,
    #     self.isosurfaces[i].radii,
    #     self.isosurfaces[i].edges,
    #     self.isosurfaces[i].polygon_normals,
    #     self.isosurfaces[i].polygon_centers,
    #     self.isosurfaces[i].intermediate_contact_polygon.vertices,
    #     self.isosurfaces[i].intermediate_contact_polygon.vertex_counts,
    #     self.isosurfaces[i].intermediate_contact_polygon.centroids,
    #     self.isosurfaces[i].contact_polygon.vertices,
    #     self.isosurfaces[i].contact_polygon.vertex_counts,
    #     self.isosurfaces[i].contact_polygon.centroids,
    #     self.isosurfaces[i].pressure_values,
    # ) =

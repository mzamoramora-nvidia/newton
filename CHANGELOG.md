...
- Add a `--load-usd` flag to the `cable_twist`, `cable_pile`, `cable_bundle_hysteresis`, and `cable_cross_slide_table` examples to optionally build the cables from bundled USD assets via `ModelBuilder.add_usd()` instead of procedurally; procedural construction remains the default.

...
- Import supported AOUSD proposal `PhysicsAttachment` prims for cable-to-xform attachments in `ModelBuilder.add_usd()`: cable `point` / `segment` sites with `type1 = "xform"` become hard point ball joints to the target xform, rigid body (including a kinematic body), or world frame and are returned in `path_attachment_map`; unsupported cloth/volume attachment sites are warned and preserved in `path_attachment_attrs`.
- Import supported AOUSD proposal `PhysicsAttachment` prims for cable-to-xform attachments in `ModelBuilder.add_usd()`: cable `point` / `segment` sites with `type1 = "xform"` become hard point ball joints to the target xform, rigid body, or world frame and are returned in `path_attachment_map`; unsupported cloth/volume attachment sites are warned and preserved in `path_attachment_attrs`.
- Import USD deformable bodies in `ModelBuilder.add_usd()` (experimental; pre-release coverage of the proposed, non-registered [AOUSD Deformable Body Physics schema](https://github.com/PixarAnimationStudios/OpenUSD-proposals/blob/5d89c0ed46a26de92f4d3fefef3bfad6500c07ce/proposals/physics_deformables/wp_deformable_physics.md)) across all three families: curve/cable (`GeomBasisCurves` + `PhysicsCurvesDeformableSimAPI` built as a VBD rod of capsule bodies and cable joints, using per-vertex normals for segment orientation; the cable joints are imported unwrapped, so the caller wraps them with `ModelBuilder.add_articulation()` before `finalize()`), surface/cloth (`GeomMesh` + `PhysicsSurfaceDeformableSimAPI` built as FEM triangles with bending edges, fan-triangulating polygonal faces such as quads), and volume (`UsdGeom.TetMesh` + `PhysicsVolumeDeformableSimAPI` built as a soft body). Bound deformable materials supply per-family thickness, stiffness, and density, and the proposal's mass-distribution precedence is honored (per-point `physics:masses`, then `PhysicsDeformableBodyAPI` `mass`/`density`, then material density). `add_usd()` returns `path_cable_map`, `path_cloth_map`, and `path_soft_map` mapping each prim path to its body/joint or particle/element ranges for per-instance addressability, plus `path_cable_attrs`, `path_cloth_attrs`, and `path_soft_attrs` exposing each deformable's as-authored, solver-neutral attributes (cable/cloth: `material` moduli and `resolved_density`; volume: `resolved_density`), including moduli the isotropic `SolverVBD` / `SolverSemiImplicit` membranes cannot apply (cable shear/twist; cloth `shearStiffness`), so an anisotropic solver such as `SolverStyle3D` can rebuild from the import.
- Import USD deformable bodies in `ModelBuilder.add_usd()` (experimental; pre-release coverage of the proposed, non-registered [AOUSD Deformable Body Physics schema](https://github.com/aousd/OpenUSD-proposals/blob/physics-deformables/proposals/physics_deformables/wp_deformable_physics.md)) across all three families: curve/cable (`GeomBasisCurves` + `PhysicsCurvesDeformableSimAPI` built as a VBD rod of capsule bodies and cable joints, using per-vertex normals for segment orientation; the cable joints are imported unwrapped, so the caller wraps them with `ModelBuilder.add_articulation()` before `finalize()`), surface/cloth (`GeomMesh` + `PhysicsSurfaceDeformableSimAPI` built as FEM triangles with bending edges, fan-triangulating polygonal faces such as quads), and volume (`UsdGeom.TetMesh` + `PhysicsVolumeDeformableSimAPI` built as a soft body). Bound deformable materials supply per-family thickness, stiffness, and density, and the proposal's mass-distribution precedence is honored (per-point `physics:masses`, then `PhysicsDeformableBodyAPI` `mass`/`density`, then material density). `add_usd()` returns `path_cable_map`, `path_cloth_map`, and `path_soft_map` mapping each prim path to its body/joint or particle/element ranges for per-instance addressability, plus `path_cable_attrs`, `path_cloth_attrs`, and `path_soft_attrs` exposing each deformable's as-authored, solver-neutral attributes (cable/cloth: `material` moduli and `resolved_density`; volume: `resolved_density`), including moduli the isotropic `SolverVBD` / `SolverSemiImplicit` membranes cannot apply (cable shear/twist; cloth `shearStiffness`), so an anisotropic solver such as `SolverStyle3D` can rebuild from the import.
...
- Add a `--load-usd` flag to the cable (`cable_twist`, `cable_pile`, `cable_bundle_hysteresis`, `cable_cross_slide_table`), cloth (`cloth_bending`, `cloth_hanging`), and soft-body (`softbody_hanging`, `rigid_soft_contact`) examples to optionally build the deformable from bundled USD assets via `ModelBuilder.add_usd()` instead of procedurally; procedural construction remains the default.
